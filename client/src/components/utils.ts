...
export function generateBarGraphFields(
  ...
  const sortedDataPoints = dataPoints
    .slice()
    .sort((a, b) =>
      a.properties.incident_date.localeCompare(b.properties.incident_date)
    );
  // Result variables
  ...
  for (let i = 1; i < sortedDataPoints.length; i++) {
    ...
    // Don't try the small optimization where this is placed in one of the blocks above,
    // that is the likely cause of Issue#2
...
export function populateIncidentList(data: any[]): IncidentType[] {
  ...
  return validCoordinatesData
    ...
    .filter(
      (incident: IncidentType | null) => incident !== null
    ) as IncidentType[];
}
